...
describe('OnChainPhotoV3', function () {
  async function fixture() {
    const [owner, user1, user2] = await ethers.getSigners();
    const contract = await ethers
      .getContractFactory('OnChainPhotoV3')
  ...
  describe('基本', function () {
    ...
    it('name&symbolが正しい', async function () {
      ...
      expect(await contract.name()).equal('OnChainPhotoV3');
      expect(await contract.symbol()).equal('OCP3');
    });
  });
